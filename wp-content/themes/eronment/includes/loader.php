...
add_action( 'init', 'eronment_bunch_theme_init');


if( !function_exists( 'eronment_set' ) ) {
	function eronment_set( $var, $key, $def = '' )
	{
		if( !$var ) return false;
...
if( !function_exists('eronment_printr' ) ) {
	function eronment_printr($data)
	{
		echo '<pre>'; print_r($data);exit;
	}
}

if( !function_exists('eronment__font_awesome' ) ) {
	function eronment_font_awesome( $index )
	{
		$array = array_values($GLOBALS['_font_awesome']);
		if( $font = eronment_set($array, $index )) return $font;
		else return false;
	}
}

if( !function_exists('eronment_load_class' ) ) {
	function eronment_load_class($class, $directory = 'libraries', $global = true, $prefix = 'Bunch_')
	{
...
eronment_load_class( 'enqueue', 'helpers', false );
eronment_load_class( 'bootstrap_walker', 'helpers', false );


if( eronment_set( $_GET, 'bunch_shortcode_editor_action' ) ) {
...
function eronment_bunch_theme_init()
{
	 ...
	 */
	// metaboxes
	$tmpl_mb1  = include ERONMENT_ROOT.'includes/vafpress/admin/metabox/meta_boxes.php';
	// * Create instances of Metaboxes
	foreach( $tmpl_mb1 as $tmb )  new VP_Metabox($tmb);
	
	$tmpl_mb1  = include ERONMENT_ROOT.'includes/vafpress/admin/taxonomy/taxonomy.php';
	...
	$tmpl_sg1  = ERONMENT_ROOT.'includes/vafpress/admin/shortcode_generator/shortcodes1.php';
	$tmpl_sg2  = ERONMENT_ROOT.'includes/vafpress/admin/shortcode_generator/shortcodes2.php';
	...
	if( is_admin() ) {
		...
		if( eronment_set( $_GET, 'page' ) == 'eronment'.'_option' ) 
		{
			if( eronment_set( $_GET, 'bunch_dummydata_export' ) ){
	...
	if( function_exists( 'eronment_vc_map' )) 
	include_once get_template_directory() .  '/vc_map.php' ;
	
	// options
	$tmpl_opt  = ERONMENT_ROOT.'includes/vafpress/admin/option/option.php';
	...
	$theme_options = new VP_Option(array(
		...
		'option_key'            => 'eronment'.'_theme_options',                      // options key in db, required
		'page_slug'             => 'eronment'.'_option',                      // options page slug, required
		...
		'page_title'            => esc_html__( 'Theme Options', 'eronment' ), // page title
		'menu_label'            => esc_html__( 'Theme Options', 'eronment' ), // menu label
	));
