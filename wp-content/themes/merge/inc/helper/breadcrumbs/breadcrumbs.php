 ...
 * Breadcrumbs
 */
if ( ! function_exists( 'merge_get_breadcrumbs' ) ) {
	function merge_get_breadcrumbs( $container_class = 'vlt-page-title__breadcrumbs' ) {
		/* === OPTIONS === */
		$text['home']     = esc_html__( 'Home', 'merge' ); // text for the 'Home' link
		$text['blog'] = esc_html__( 'Blog', 'merge' ); // text for the 'Blog' link
		$text['category'] = esc_html__( 'Archive by Category "%s"', 'merge' ); // text for a category page
		$text['search']   = esc_html__( 'Search Results for "%s"', 'merge' ); // text for a search results page
		$text['tag']      = esc_html__( 'Posts Tagged "%s"', 'merge' ); // text for a tag page
		$text['author']   = esc_html__( 'Articles Posted by %s', 'merge' ); // text for an author page
		$text['404']      = esc_html__( 'Error 404', 'merge' ); // text for the 404 page

		/* === OPTIONS === */
		// $text['blog']     = esc_html__( 'Blog', 'merge' ); // text for the 'Home' link
		// if( get_option( 'page_for_posts' ) !=='0' ){
		// $text['category'] = '<a href="'.get_permalink( get_option( 'page_for_posts' ) ) . '">' . esc_html__( 'Blog', 'merge' ) . '</a> <span class="sep">-</span> ' . esc_html__( '%s', 'merge' ); // text for a category page
		// }
		// $text['search']   = esc_html__( 'Search Results for "%s"', 'merge' ); // text for a search results page
		// $text['tag']      = esc_html__( '%s', 'merge' ); // text for a tag page
		// $text['author']   = esc_html__( 'Articles Posted by %s', 'merge' ); // text for an author page
		// $text['404']      = esc_html__( '404', 'merge' ); // text for the 404 page
		// $text['page']     = esc_html__( 'Page %s', 'merge' ); // text 'Page N'
		// $text['cpage']    = esc_html__( 'Comment Page %s', 'merge' ); // text 'Comment Page N'
		$wrap_before      = '<nav class="' . merge_sanitize_class( $container_class ) . '">'; // the opening wrapper tag
		$wrap_after       = '</nav>'; // the closing wrapper tag
		$sep              = merge_get_icon( 'dot' ); // separator between crumbs
		...
		if ( is_home() || is_front_page() ) {
			if ( $show_on_home ) {
				$output .= $wrap_before . '<a href="' . $home_link . '">' . $text['home'] . '</a>' . $sep . $link_before . $text['blog'] . $link_after . $wrap_after;
			}
		} else {
			$output .= $wrap_before;
			if ( $show_home_link ) {
			...
			if ( is_category() ) {
				...
				if ( get_post_type() != 'post' ) {
					...
					if ( get_option( 'page_for_posts' ) !=='0' ) {
						$output .= '<a href="'.get_permalink( get_option( 'page_for_posts' ) ) . '">' . esc_html__( 'Blog', 'merge' ) . '</a> <span class="sep">-</span> '. $cats;
					} else {
						$output .= $cats;
					}
